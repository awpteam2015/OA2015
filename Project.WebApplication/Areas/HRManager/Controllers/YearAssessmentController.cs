...
{
    public class YearAssessmentController : BaseController
    {
        ...
        public AbpJsonResult GetList()
        {
            ...
            var where = new YearAssessmentEntity();
            ...
            var searchList = YearAssessmentService.GetInstance().Search(where, (pIndex - 1) * pSize, pSize);
        ...
        public AbpJsonResult GetAllList()
        {
            ...
            //where.DepartmentCode = RequestHelper.GetFormString("DepartmentCode");
            var searchList = YearAssessmentService.GetInstance().GetList(where);
        ...
        [HttpPost]
        public AbpJsonResult Add(AjaxRequest<YearAssessmentEntity> postData)
        {

            postData.RequestEntity.EmployeeID = postData.RequestEntity.PkId;
            postData.RequestEntity.PkId = 0;
            var addResult = YearAssessmentService.GetInstance().Add(postData.RequestEntity);
            var result = new AjaxResponse<YearAssessmentEntity>()
            {
                success = true,
        ...
        [HttpPost]
        public AbpJsonResult Edit(AjaxRequest<YearAssessmentEntity> postData)
        {
            var newInfo = postData.RequestEntity;
            var orgInfo = YearAssessmentService.GetInstance().GetModelByPk(postData.RequestEntity.PkId);
            var mergInfo = Mapper.Map(newInfo, orgInfo);
            var updateResult = YearAssessmentService.GetInstance().Update(mergInfo);
        ...
        {
            var deleteResult = YearAssessmentService.GetInstance().DeleteByPkId(pkid);
            var result = new AjaxResponse<YearAssessmentEntity>()
            {
