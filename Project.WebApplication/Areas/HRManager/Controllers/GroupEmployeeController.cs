...
namespace Project.WebApplication.Areas.HRManager.Controllers
{
    public class GroupEmployeeController : BaseController
    {
        ...
        public ActionResult Hd(int pkId = 0)
        {
            if (pkId > 0)
            {
                var entity = GroupEmployeeService.GetInstance().GetModelByPk(pkId);
        ...
        [HttpPost]
        public AbpJsonResult Add(AjaxRequest<GroupEmployeeEntity> postData)
        {
            var addResult = GroupEmployeeService.GetInstance().Add(postData.RequestEntity);
        ...
        {
            var deleteResult = GroupEmployeeService.GetInstance().DeleteByPkId(pkid);
            var result = new AjaxResponse<GroupEmployeeEntity>()
            {
