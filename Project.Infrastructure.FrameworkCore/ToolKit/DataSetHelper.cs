...
*****************************************************
修改人员：李伟伟               修改日期：2009-07-26
修改说明：
 * InsertGroupByInto 方法 field.Aggregate 如果为null 则field.Aggregate.ToLower() 出错，去除.ToLower()
 * (decimal) a + (decimal) b 转换会出错 改为: Convert.ToDecimal(a) + Convert.ToDecimal(b)
*******************************************************/
    ...
    public class DataSetHelper
    {
                                       ...
                                       string rowFilter, string groupBy)
        {
            ...
            foreach (DataRow sourceRow in rows)
            {
                ...
                foreach (FieldInfo field in GroupByFieldInfo)
                {
                    switch (field.Aggregate)  //InsertGroupByInto 方法 field.Aggregate 如果为null 则field.Aggregate.ToLower() 出错 
                    {
                        case null:
                        case "":
                        case "last":
        ...
        private object Add(object a, object b)
        {
            ...
            return (Convert.ToDecimal(a) + Convert.ToDecimal(b));  //(decimal) a + (decimal) b 改为: Convert.ToDecimal(a) + Convert.ToDecimal(b) 
